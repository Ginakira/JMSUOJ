...
<h2 style="margin:0px 10px">Post New Thread<?php if (array_key_exists('cid', $_REQUEST) && $_REQUEST['cid']!='') {
    echo ' For Contest '.intval($_REQUEST['cid']);
}?></h2>
<form action="post.php?action=new" method=post>
<input type=hidden name=cid value="<?php if (array_key_exists('cid', $_REQUEST)) {
