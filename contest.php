...
if (isset($_GET['cid'])) {
    ...
    foreach ($result as $row) {
        ...
        $view_problemset[$cnt][1] = "";
        if ($OJ_CONTEST_SHOWID) {
            $view_problemset[$cnt][1] .= $row['problem_id'] . " ";
        }
...
//    echo $sql;
    ...
    foreach ($result as $row) {
        ...
        if ($now > $end_time) {
            $view_contest[$i][2] = "<span class='text-muted'><span class='badge badge-secondary'>$MSG_Ended</span> " . $row['end_time'] . "</span>";
            //pending

        } else if ($now < $start_time) {
            $view_contest[$i][2] = "<span class='text-info'><span class='badge badge-info'>$MSG_Start</span> " . $row['start_time'] . "</span>&nbsp;";
            $view_contest[$i][2] .= "<span class='small text-muted'>$MSG_TotalTime" . formatTimeLength($length) . "</span>";
            //running
        } else {
            $view_contest[$i][2] = "<span class='badge badge-success'>$MSG_Running</span>&nbsp;";
            $view_contest[$i][2] .= "<span class='text-success'> $MSG_LeftTime " . formatTimeLength($left) . " </span>";
        }

        $private = intval($row['private']);
        if ($private == 0) {
            $view_contest[$i][4] = "<span class='badge badge-info'>$MSG_Public</span>";
        } else {
            $view_contest[$i][5] = "<span class='badge badge-warning'>$MSG_Private</span>";
        }
