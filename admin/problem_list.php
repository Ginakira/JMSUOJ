    ...
    <center>
        <table width=100% border=1 style="text-align:center;">
            <form method=post action=contest_add.php>
                <tr>
                    <td width=60px>ID <input type=checkbox style='vertical-align:2px;'
                                             onchange='$("input[type=checkbox]").prop("checked", this.checked)'></td>
                    <td>题目名</td>
                    <td>AC数</td>
                    <td>更新时间</td>
                    <?php
                ...
                foreach ($result as $row) {
                    ...
                    if (isset($_SESSION[$OJ_NAME . '_' . 'administrator']) || isset($_SESSION[$OJ_NAME . '_' . 'problem_editor'])) {
                        // 20200526 暂时修改为所有题目管理员可更改
                        // 原题目创建者检测 isset($_SESSION[$OJ_NAME . '_' . "p" . $row['problem_id']])
                        echo "<td><a href=problem_df_change.php?id=" . $row['problem_id'] . "&getkey=" . $_SESSION[$OJ_NAME . '_' . 'getkey'] . ">" . ($row['defunct'] == "N" ? "<span titlc='click to reserve it' class=green>已启用</span>" : "<span class=red title='click to be available'>已禁用</span>") . "</a></td>";
                        if (isset($_SESSION[$OJ_NAME . '_' . 'administrator'])) {
                            if ($OJ_SAE || function_exists("system")) {
                                ?>
                                <td>
                                    <a href=#
                                       onclick='javascript:if(confirm("您确定要删除吗?【请谨慎操作】")) location.href="problem_del.php?id=<?php echo $row['problem_id'] ?>&getkey=<?php echo $_SESSION[$OJ_NAME . '_' . 'getkey'] ?>"'>删除</a>
                                </td>
                                <?php
                            }
                        }
                        ...
                        if (isset($_SESSION[$OJ_NAME . '_' . 'administrator']) || isset($_SESSION[$OJ_NAME . '_' . 'problem_editor'])) {
                            ...
                            echo "<td><a href='javascript:phpfm(" . $row['problem_id'] . ");'>编辑数据</a>";
                        }
                    }
                    echo "</tr>";
                }
                ?>
                <tr>
                    <td colspan=2 style="height:40px;">操作</td>
                    <td colspan=6>
                        <input type=submit name='problem2contest' value='新比赛/作业'>
                        <input type=submit name='enable' value='启用'
                               onclick='$("form").attr("action","problem_df_change.php")'>
                        <input type=submit name='disable' value='禁用'
                               onclick='$("form").attr("action","problem_df_change.php")'>
                    </td>
                </tr>
            </form>
