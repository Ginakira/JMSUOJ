...
if ($OJ_TEMPLATE != "classic") {
    $judge_color = array("label gray", "badge badge-info", "badge badge-warning", "badge badge-warning", "badge badge-success", "badge badge-danger", "badge badge-danger", "badge badge-warning", "badge badge-warning", "badge badge-warning", "badge badge-warning", "badge badge-warning", "badge badge-warning", "badge badge-info");
}
...
for ($i = 0; $i < $rows_cnt; $i++) {
        ...
        if (!$lock || $lock_time > $row['in_date'] || $row['user_id'] == $_SESSION[$OJ_NAME . '_' . 'user_id']) {
            if ($OJ_SIM && $row['sim'] > 80 && $row['sim_s_id'] != $row['s_id']) {
                ...
                if (isset($_SESSION[$OJ_NAME . '_' . 'source_browser'])) {
                    ...
                    $view_status[$i][3] .= "<a href=comparesource.php?left=" . $row['sim_s_id'] . "&right=" . $row['solution_id'] . "  class='badge badge-info'  target=original>" . $row['sim_s_id'] . "(" . $row['sim'] . "%)</a>";
                } else {

                    $view_status[$i][3] .= "<span class='badge badge-info'>" . $row['sim_s_id'] . "</span>";
    ...
    if ($flag) {
        ...
        if (!(isset($_SESSION[$OJ_NAME . '_' . 'user_id']) && strtolower($row['user_id']) == strtolower($_SESSION[$OJ_NAME . '_' . 'user_id']) || isset($_SESSION[$OJ_NAME . '_' . 'administrator']))) {
            //原为sorcebroswer 但与administrator冲突导致多个权限无法查看代码 临时修改为administrator
